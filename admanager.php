	...
	else
	{
		$currentAd = array("campaignID" 	=> "", 
				   		   "configArray" 	=> array("Method" 							=> "windowtoplocation",
				   		   							 ...
				   		   							 "CloakTrackingPixelEnabled"		=> "false",
				   		   							 "OutputMethod" 					=> "HTML", 
...
<?php
	...
	else if (!empty($_GET['copy']) || !empty($_GET['copyProfile']))
	{
		echo "toastr.success('Copied');\n";
	}
	else if (!empty($_GET['delete']) || !empty($_GET['deleteProfile']))
	{
...
<?php
	...
	else if (array_key_exists("edit", $_GET) || array_key_exists("newprofile", $_GET) || array_key_exists("editprofile", $_GET))
	{

		$redirectMethodOptions = array("trycatchredirect", "windowlocation", "windowtoplocation", "windowtoplocationtoiframepage", "windowtoplocationhref", "parentwindowlocationhref", "0x0iframe", "1x1iframe");
...
?>
		...
		<fieldset>
			...
			<tr>
				<td class="col-xs-5">Cloak Tracking Pixel URL</td>
				<td><input type="text" name="CloakTrackingPixelUrl" id="CloakTrackingPixelUrl" class="form-control form-control-lg" value="<?= array_get_value_with_default($currentAd["configArray"], "CloakTrackingPixelUrl"); ?>" /></td>
			</tr>

			<tr>
				<td>Cloak Tracking Pixel Enabled</td>
				<td>
					<input type="hidden" name="CloakTrackingPixelEnabled" value="false" />
					<input class="form-check-input" type="checkbox" name="CloakTrackingPixelEnabled" value="true" <?= (array_get_bool($currentAd["configArray"], "CloakTrackingPixelEnabled") ? "checked=checked" : null); ?> />
				</td>
			</tr>
...
<?php
		...
		else if (array_key_exists("HTMLTemplate", $currentAd["configArray"]))
		{
?>
...
<?php 
	...
	else if (array_key_exists("runtest", $_GET))
	{
		echo "run test";
	}
	else if (array_key_exists("viewlog", $_GET))
	{
